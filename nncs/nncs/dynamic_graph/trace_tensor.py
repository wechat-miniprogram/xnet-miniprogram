...
class TensorMeta:
    ...
    def default_comparator(lhs: "TensorMeta", rhs: "TensorMeta"):
        return (
            lhs.index == rhs.index
            and lhs.creator_id == rhs.creator_id
    ...
    def __init__(self, creator_id, index, shape):
        ...
        )  # Handle cases when shape is a tuple of Tensors
    ...
    def __str__(self):
        return (
            "C{}_I{}_".format(self.creator_id, self.index)
            + "S"
...
class TracedTensor(torch.Tensor):
    ...
    # NOTE: This disables the __torch_function__ API altogether when using NNCS.
    # TODO: make NNCS utilize the __torch_function__ API instead.
...
def trace_tensors(operator_output, node: "NNCSNode"):
...
def make_input_infos(inputs: "OperatorInput"):
    ...
    for i, node_input_index_entry in enumerate(inputs):
        ...
        elif isinstance(node_input, torch.Tensor) and not isinstance(
            node_input, TracedTensor
        ):
            meta = TensorMeta(None, i, node_input.shape)
            input_infos.append(meta)
        elif isinstance(node_input, numbers.Number):
            tensor = torch.Tensor([node_input])
            meta = TensorMeta(None, i, tensor.shape)
            meta.value = tensor
            input_infos.append(meta)
        else:
