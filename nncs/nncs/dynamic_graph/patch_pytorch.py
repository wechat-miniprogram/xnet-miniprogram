...
from nncs.dynamic_graph.trace_tensor import TracedTensor, flatten_args
from nncs.dynamic_graph.wrappers import wrap_operator, wrap_module_call, ignore_scope
...
class ForwardTraceOnly(CustomTraceFunction):
    def __call__(self, operator: Callable, *args, **kwargs):
        ...
        compression, but still have to be accounted for so that the NNCS internal graph representation
...
def torch_jit_script_wrapper(*args, **kwargs):
    # Torch JIT cannot work with NNCS-modified operators,
...
def get_arg_positions_to_quantize(op_name: str):
    from nncs.dynamic_graph.function_input_quantization import FUNCTIONS_TO_QUANTIZE
...
def patch_torch_operators():
    ...
    import torch
    from nncs.dynamic_graph.operator_metatypes import OPERATOR_METATYPES
    for op_meta_class in OPERATOR_METATYPES.registry_dict.values():  # type: OperatorMetatype
        ...
        if op_meta_class.torch_tensor_patch_spec is not None:
            ...
            # patch_namespace_by_patchspec(Tensor, ps)
