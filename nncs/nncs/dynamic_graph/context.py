...
from nncs.dynamic_graph.graph import InputAgnosticOperationExecutionContext
from nncs.dynamic_graph.graph import NNCSGraph, NNCSNode
from nncs.dynamic_graph.trace_tensor import make_input_infos
from nncs.dynamic_graph.version_agnostic_op_names import get_version_agnostic_name
from nncs.utils import maybe_get_iterator
...
class InputIndexEntry:
    def __init__(
        self, path: Tuple[Union[int, str], ...], getter: Callable, setter: Callable
    ):
        self.path = path
        self.getter = getter
        self.setter = setter
...
class TupleRebuildingSetter:
    def __init__(
        self, idx_to_set, current_tuple, previous_level_setter_for_current_tuple
    ):
...
class OperatorInput:
    def __init__(self, op_args, op_kwargs):
        ...
        self._nested_object_paths_generator(
            self.op_args,
            op_args_index_entries,
        ...
        self._index = {
            idx: entry
            for idx, entry in enumerate(op_args_index_entries + op_kwargs_index_entries)
        }

    @staticmethod
    def _nested_object_paths_generator(
        obj, out_entries_list, path=(), memo=None, previous_level_setter=None
    ):
        if memo is None:
            memo = set()
        ...
        if iterator is not None:
            if id(obj) not in memo:
                ...
                for idx, iterval in enumerate(iterator(obj)):
                    ...
                    current_level_getters.append(
                        partial(obj.__getitem__, path_component)
                    )
                    if not isinstance(obj, tuple):
                        current_level_setters.append(
                            partial(obj.__setitem__, path_component)
                        )
                    else:
                        current_level_setters.append(
                            TupleRebuildingSetter(idx, obj, previous_level_setter)
                        )

                for idx, iterval in enumerate(iterator(obj)):
                    path_component, value = iterval
                    retval = OperatorInput._nested_object_paths_generator(
                        value,
                        out_entries_list,
                        path + (path_component,),
                        memo,
                    ...
                    if was_leaf:
                        ...
                        out_entries_list.append(
                            InputIndexEntry(leaf_entry_path, getter, setter)
                        )
...
class ScopeElement:
    ...
    def __str__(self):
        ...
        return "{cls}[{name}]".format(
            cls=self.calling_module_class_name, name=self.calling_field_name
        )
...
class TracingContext:
    ...
    def find_operator_node(
        ...
        node = self.graph.find_node(
            ia_op_exec_context, tensor_metas, self._input_comparators_per_scope
        )
    ...
    def get_caller_context(
        ...
        usually handled inside NNCS. This mechanism allows to discern between multiple function calls inside the same
        ...
        call_order = self.get_operator_call_count_in_scope(
            version_agnostic_operator_type, self.scope
        )

        ia_op_exec_context = InputAgnosticOperationExecutionContext(
            version_agnostic_operator_type, self.scope, call_order
        )
    ...
    def push_scope(self, scope_module):
        relative_scopes_list = self._get_scope_relative_to_last_registered_module_call(
            scope_module
        )
    ...
    def register_pre_hooks(
        ...
        if pre_hook_id in self._pre_hooks:
            raise KeyError(
                "Pre hook for context {} is already registered".format(str(pre_hook_id))
            )
    ...
    def execute_pre_hooks(
        self,
        ia_op_exec_context: InputAgnosticOperationExecutionContext,
        op_inputs: OperatorInput,
    ) -> OperatorInput:
        in_op = getattr(self, "in_operator", False)
        self.in_operator = False
        self._thread_local.num_nested_hooks += 1

        pre_hook_ids_for_curr_op = [
            x for x in self._pre_hooks if x.ia_op_exec_context == ia_op_exec_context
        ]
        pre_hook_ids_for_curr_op = sorted(
            pre_hook_ids_for_curr_op, key=lambda x: x.input_port_id
        )
    ...
    def register_post_hooks(
        ...
        if ia_op_exec_context in self._post_hooks:
            raise KeyError(
                "Post hook for context {} is already registered".format(
                    str(ia_op_exec_context)
                )
            )
        self._post_hooks[ia_op_exec_context] = fn_list

    def execute_post_hooks(
        self, ia_op_exec_context: InputAgnosticOperationExecutionContext, outputs
    ):
        in_op = getattr(self, "in_operator", False)
        self.in_operator = False
        self._thread_local.num_nested_hooks += 1
    ...
    def add_node_comparators(
        self,
        scopes_to_apply: List[str],
        node_input_comparator: "TensorMetaComparator" = None,
    ):
        self._input_comparators_per_scope.append(
            (node_input_comparator, scopes_to_apply)
        )
