...
from nncs.dynamic_graph.graph_matching import (
    Expression,
    NodeExpression,
    search_all,
    get_edge_boundaries,
)
from nncs.dynamic_graph.trace_tensor import TensorMeta, TracedTensor
...
class InputsMatcher:
    def __call__(
        self,
        node_inputs: List[TensorMeta],
        real_inputs: List[TensorMeta],
...
class InputAgnosticOperationExecutionContext:
    ...
    def from_str(s: str):
        ...
        from nncs.dynamic_graph.context import Scope
...
class OperationExecutionContext:
    """Information that allows to uniquely identify an operation inside the NNCS graph,
    ...
    def __init__(
        ...
        self.input_agnostic = InputAgnosticOperationExecutionContext(
            operator_name, scope_in_model, call_order
        )
        ...
        self.tm_comparators = (
            tm_comparators if tm_comparators else [DefaultTensorMetaComparator()]
        )
    ...
    def __eq__(self, other: "OperationExecutionContext"):
        return (self.input_agnostic == other.input_agnostic) and self.input_matcher(
            self.tensor_metas, other.tensor_metas, self.tm_comparators
        )
...
class NNCSNode:
    def __init__(
        ...
        module_attributes: BaseModuleAttributes = None,
    ):
...
class DefaultScopeNodeMatcher:
    def __init__(self, node_id_to_key_dict, nx_graph, nx_node_to_nncs_node):
        self._node_id_to_key_dict = node_id_to_key_dict
        self._nx_graph = nx_graph
        self._nx_node_to_nncs_node = nx_node_to_nncs_node
    ...
    def _find_nodes_with_matching_context_among_inputless(
        ...
        for nx_node_key, nx_node in self._inputless_nx_nodes.items():
            if nx_node[NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR] == op_exec_context:
    ...
    def _find_nodes_with_matching_context_and_inputs(
        ...
        for info in op_exec_context.tensor_metas:
            ...
            for successor_node_key in self._nx_graph.successors(
                self._node_id_to_key_dict[creator_id]
            ):
                successor_node = self._nx_graph.nodes[successor_node_key]
                if (
                    op_exec_context
                    == successor_node[NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR]
                ):
                    node_candidates[successor_node_key] = successor_node
        return node_candidates

    def add_node(self, op_exec_context: OperationExecutionContext, inputs) -> NNCSNode:
        node_id = len(self._node_id_to_key_dict)

        name_parts = (
            str(op_exec_context.scope_in_model),
            op_exec_context.operator_name,
        )
        node_key = "{idx} {uri}".format(uri="/".join(name_parts), idx=node_id)

        nncs_logger.debug("New node added to NNCS graph: {}".format(node_key))

        self._node_id_to_key_dict[node_id] = node_key
        attrs = {
            NNCSGraph.ID_NODE_ATTR: node_id,
            NNCSGraph.KEY_NODE_ATTR: node_key,
            NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR: op_exec_context,
        }
        ...
        for i, info in enumerate(op_exec_context.tensor_metas):
            ...
            self._nx_graph.edges[parent, node_key][
                NNCSGraph.ACTIVATION_SHAPE_EDGE_ATTR
            ] = info.shape
            self._nx_graph.edges[parent, node_key][NNCSGraph.IN_PORT_NAME_EDGE_ATTR] = i

        if not has_traced_inputs:
            self._inputless_nx_nodes[node_key] = self._nx_graph.nodes[node_key]

        return self._nx_node_to_nncs_node(self._nx_graph.nodes[node_key])
    ...
    def find_node(
        ...
        for nx_node in node_candidates.values():
            nncs_node_candidates.append(
                NNCSNode(
                    nx_node[NNCSGraph.ID_NODE_ATTR],
                    op_exec_context,
                    nx_node.get(NNCSGraph.MODULE_ATTRIBUTES),
                )
            )
        result = None
        if len(nncs_node_candidates) == 1:
            result = nncs_node_candidates[0]
        if len(nncs_node_candidates) > 1:
            nncs_logger.warning("More than one node matches input")
            result = nncs_node_candidates[0]
...
class IterationScopeNodeMatcher(DefaultScopeNodeMatcher):
    def __init__(self, node_id_to_key_dict, nx_graph, nx_node_to_nncs_node):
        super().__init__(node_id_to_key_dict, nx_graph, nx_node_to_nncs_node)
        self._first_iteration_nodes = {}  # type: {str: {str: NNCSNode}}

    def save_first_iteration_node(self, inputs, node: NNCSNode):
        """
        ...
        if iter_scopes:
            for iter_scope in iter_scopes:
                ...
                for i in inputs:
                    ...
                    within_scopes = (
                        creator_node_op_exec_ctx.scope_in_model.get_iteration_scopes()
                    )
        ...
        if not node_candidates:
            ...
            if not node_candidates and iter_scopes:
                ...
                for node in iter_nodes.items():
                    nncs_node_candidates.append(node[1])
    ...
    def _match_first_iteration_nodes(
        self, op_exec_context: OperationExecutionContext, iter_scopes
    ):
        node_candidates = {}
        for iter_scope in iter_scopes:
...
class NodeManager:
    def __init__(self, node_id_to_key_dict, nx_graph, nx_node_to_nncs_node):
        self.base_matcher = DefaultScopeNodeMatcher(
            node_id_to_key_dict, nx_graph, nx_node_to_nncs_node
        )
        self.iteration_matcher = IterationScopeNodeMatcher(
            node_id_to_key_dict, nx_graph, nx_node_to_nncs_node
        )
    ...
    def choose_matcher(
        self, ia_op_exec_context: InputAgnosticOperationExecutionContext
    ) -> DefaultScopeNodeMatcher:
    ...
    def find_node(
        ...
        comparators = self.choose_tm_comparators(
            ia_op_exec_context, input_comparators_per_scope
        )
    ...
    def add_node(
        ...
        matcher = self.choose_matcher(ia_op_exec_context)
        tm_comparators = self.choose_tm_comparators(
            ia_op_exec_context, tm_comparators_per_scope
        )
        op_exec_context = OperationExecutionContext(
            ia_op_exec_context.operator_name,
            ia_op_exec_context.scope_in_model,
...
class NNCSGraphEdge:
    def __init__(self, from_node: NNCSNode, to_node: NNCSNode, tensor_shape: Tuple):
...
class NNCSGraphPatternIO:
    def __init__(
        self,
        input_edges: List[NNCSGraphEdge],
        output_edges: List[NNCSGraphEdge],
        input_nodes: List[NNCSNode],
        output_nodes: List[NNCSNode],
    ):
...
class NNCSGraph:
    ...
    def __init__(self):
        ...
        self.match_manager = NodeManager(
            self._node_id_to_key_dict, self._nx_graph, self._nx_node_to_nncs_node
        )
        self._input_nncs_nodes = []

    def __eq__(self, other: "NNCSGraph"):
        nm = iso.categorical_node_match(
            [
                NNCSGraph.ID_NODE_ATTR,
                NNCSGraph.KEY_NODE_ATTR,
                NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR,
            ],
            [None, None, None],
        )
        em = iso.categorical_edge_match(
            [NNCSGraph.ACTIVATION_SHAPE_EDGE_ATTR, NNCSGraph.IN_PORT_NAME_EDGE_ATTR],
            [None, None],
        )
        return nx.is_isomorphic(
            self._nx_graph, other._nx_graph, node_match=nm, edge_match=em
        )
    ...
    def find_node(
        ...
        return self.match_manager.find_node(
            ia_op_exec_context, tensor_metas, input_comparators_per_scope
        )
    ...
    def add_node(
        ...
        node = self.match_manager.add_node(
            ia_op_exec_context, tensor_metas, input_comparators_per_scope, inputs
        )

        from nncs.dynamic_graph.input_wrapping import MODEL_INPUT_OP_NAME

        if (
            node.op_exec_context.operator_name == MODEL_INPUT_OP_NAME
        ):  # TODO: refactorable model input node name
            self._input_nncs_nodes.append(node)
        return node

    def get_nx_node_by_key(self, key: str):
        return self._nx_graph.nodes[key]

    def get_nncs_node_by_id(self, node_id: int) -> NNCSNode:
        nx_node = self.get_nx_node_by_key(self.get_node_key_by_id(node_id))
        nncs_node = self._nx_node_to_nncs_node(nx_node)
        return nncs_node

    def get_node_key_by_iap_context(
        self, iap_ctx: InputAgnosticOperationExecutionContext
    ) -> str:
        for node_key, node in self._nx_graph.nodes.items():
            if node[NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR].input_agnostic == iap_ctx:
                return node_key
    ...
    def get_node_by_id(self, node_id):
        return self._nx_node_to_nncs_node(
            self._nx_graph.nodes[self.get_node_key_by_id(node_id)]
        )

    def get_matching_nncs_graph_pattern_io_list(
        self, expression: Expression
    ) -> List[NNCSGraphPatternIO]:
        matched_node_key_sequences = search_all(self._nx_graph, expression)
        pattern_ios = [
            self._get_nncs_graph_pattern_io_list(match)
    ...
    def is_output_node(self, node: NNCSNode) -> bool:
        return not list(
            self._nx_graph.successors(self._node_id_to_key_dict[node.node_id])
        )

    def get_nx_graph_copy(self) -> nx.DiGraph:
        return deepcopy(self._nx_graph)

    def get_input_nodes(self) -> List[NNCSNode]:
        return self._input_nncs_nodes

    def get_graph_outputs(self) -> List[NNCSNode]:
        outputs = []
        for nx_node_key, deg in self._nx_graph.out_degree():
            if deg == 0:
                outputs.append(
                    self._nx_node_to_nncs_node(self._nx_graph.nodes[nx_node_key])
                )
        return outputs

    # pylint:disable=protected-access
    def get_nx_edge(self, node_u: NNCSNode, node_v: NNCSNode):
    ...
    def get_next_nodes(self, node: NNCSNode) -> Optional[List[NNCSNode]]:
        nx_node_keys = self._nx_graph.succ[self._node_id_to_key_dict[node.node_id]]
        return [
            self._nx_node_to_nncs_node(self._nx_graph.nodes[key])
            for key in nx_node_keys
        ]

    def get_previous_nodes(self, node: NNCSNode) -> Optional[List[NNCSNode]]:
    ...
    def get_inputs_count(self, node: NNCSNode) -> int:
        return self._nx_graph.in_degree()[self._node_id_to_key_dict[node.node_id]]

    def traverse_graph(
        self,
        curr_node: NNCSNode,
        traverse_function: Callable[[NNCSNode], Tuple[bool, List[Any]]],
        traverse_forward: bool = True,
    ):
        output = []
        return self._traverse_graph_recursive_helper(
            curr_node, traverse_function, output, traverse_forward
        )

    def _traverse_graph_recursive_helper(
        self,
        curr_node: NNCSNode,
        traverse_function: Callable[[NNCSNode, List[Any]], Tuple[bool, List[Any]]],
        output: List[Any],
        traverse_forward: bool,
    ):
        is_finished, output = traverse_function(curr_node, output)
        get_nodes_fn = (
            self.get_next_nodes if traverse_forward else self.get_previous_nodes
        )
        if not is_finished:
            for node in get_nodes_fn(curr_node):
                self._traverse_graph_recursive_helper(
                    node, traverse_function, output, traverse_forward
                )
        return output
    ...
    def node_type_fn(node: dict) -> str:
        return node[NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR].operator_name

    def get_output_shapes_for_ia_op_exec_context(
        self, ia_op_exec_context: InputAgnosticOperationExecutionContext
    ) -> List[Tuple]:
        node_key = self.get_node_key_by_iap_context(ia_op_exec_context)
        succs = list(self._nx_graph.successors(node_key))
        edge_list = [
            self._nx_graph.edges[node_key, to_node_key] for to_node_key in succs
        ]
        return [edge[NNCSGraph.ACTIVATION_SHAPE_EDGE_ATTR] for edge in edge_list]

    def get_input_shapes_for_ia_op_exec_context(
        self, ia_op_exec_context: InputAgnosticOperationExecutionContext
    ) -> Dict[int, Tuple]:
        node_key = self.get_node_key_by_iap_context(ia_op_exec_context)
        ...
        for in_edge in in_edges:
            ...
            port_id = edge_attr_dict[NNCSGraph.IN_PORT_NAME_EDGE_ATTR]
            assert port_id not in retval
            retval[port_id] = edge_attr_dict[NNCSGraph.ACTIVATION_SHAPE_EDGE_ATTR]
        return retval
    ...
    def _get_graph_for_structure_analysis(self, extended=False) -> nx.DiGraph:
        ...
        for node_name, node in self._nx_graph.nodes.items():
            op_exec_context = node[NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR]
            scope_str = str(op_exec_context.scope_in_model)
            attrs_node = {
                "type": op_exec_context.operator_name,
                "id": node[NNCSGraph.ID_NODE_ATTR],
                "scope": scope_str,
            }
            if "color" in node:
        ...
        if extended:
            for u, v in self._nx_graph.edges:
                out_graph.add_edge(
                    u,
                    v,
                    label=self._nx_graph.edges[u, v][
                        NNCSGraph.ACTIVATION_SHAPE_EDGE_ATTR
                    ],
                )
        else:
    ...
    def _get_graph_for_visualization(self) -> nx.DiGraph:
        ...
        for node_name, node in self._nx_graph.nodes.items():
            ...
            attrs_node = {}
            attrs_node["label"] = (
                str(node[NNCSGraph.ID_NODE_ATTR])
                + " "
                + str(op_exec_context.input_agnostic)
            )
        ...
        for u, v in self._nx_graph.edges:
            out_graph.add_edge(
                u,
                v,
                label=self._nx_graph.edges[u, v][NNCSGraph.ACTIVATION_SHAPE_EDGE_ATTR],
            )
    ...
    def _get_topologically_last_nodes(self, matches: List[List[str]]) -> List[str]:
        topological_order = {
            node: k for k, node in enumerate(nx.topological_sort(self._nx_graph))
        }
        insertion_points = {
            max(match, key=topological_order.__getitem__) for match in matches
        }
    ...
    def _get_nncs_graph_pattern_input_output(
        self, match: List[str]
    ) -> NNCSGraphPatternIO:
        ...
        for key in match:
            ...
            # A proper way to find the input nodes would be to mark the tensors arriving at NNCSNetwork's
            ...
            if not list(self._nx_graph.successors(key)):
                output_nncs_nodes.append(
                    self._nx_node_to_nncs_node(self._nx_graph.nodes[key])
                )
            if not list(self._nx_graph.predecessors(key)):
                input_nncs_nodes.append(
                    self._nx_node_to_nncs_node(self._nx_graph.nodes[key])
                )
        ...
        for nx_edge in boundary:
            ...
            data = nx_edge[2]
            nncs_edge = NNCSGraphEdge(
                self._nx_node_to_nncs_node(self._nx_graph.nodes[from_node_key]),
                self._nx_node_to_nncs_node(self._nx_graph.nodes[to_node_key]),
                data[NNCSGraph.ACTIVATION_SHAPE_EDGE_ATTR],
            )
            if from_node_key in match:
                output_nncs_edges.append(nncs_edge)
            elif to_node_key in match:
                input_nncs_edges.append(nncs_edge)
            else:
                raise RuntimeError("Invalid graph expression supplied!")

        return NNCSGraphPatternIO(
            input_nncs_edges, output_nncs_edges, input_nncs_nodes, output_nncs_nodes
        )

    def _get_nncs_graph_pattern_io_list(self, match: List[str]) -> NNCSGraphPatternIO:
    ...
    @staticmethod
    def _nx_node_to_nncs_node(nx_node) -> "NNCSNode":
        return NNCSNode(
            nx_node[NNCSGraph.ID_NODE_ATTR],
            nx_node[NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR],
            nx_node.get(NNCSGraph.MODULE_ATTRIBUTES),
        )
    ...
    def find_node_in_nx_graph_by_scope(self, scope: "Scope") -> Optional[dict]:
        ...
        for node_key in nodes:
            if (
                nodes[node_key][NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR].scope_in_model
                == scope
            ):
    ...
    def get_op_nodes_in_scope(self, scope: "Scope") -> List:
        ...
        for _, node in self._nx_graph.nodes.items():
            op_scope = node[
                NNCSGraph.OP_EXEC_CONTEXT_NODE_ATTR
            ].input_agnostic.scope_in_model
            if op_scope in scope:
                matching_graph_op_nodes.append(node)
        return matching_graph_op_nodes

    def get_nodes_by_types(self, type_list: List[str]) -> List[NNCSNode]:
        all_nodes_of_type = []
        for node_key in self.get_all_node_keys():
            ...
            if node_type in type_list:
                all_nodes_of_type.append(nncs_node)
        return all_nodes_of_type

    def get_all_nodes(self) -> List[NNCSNode]:
        all_nodes = []
        for node_key in self.get_all_node_keys():
            nx_node = self._nx_graph.nodes[node_key]
            nncs_node = self._nx_node_to_nncs_node(nx_node)
            all_nodes.append(nncs_node)
        return all_nodes


class NNCSNodeExpression(NodeExpression):
    def __init__(self, node_type: str = None, filter_fn=None):
        super().__init__(node_type, filter_fn, node_type_fn=NNCSGraph.node_type_fn)
