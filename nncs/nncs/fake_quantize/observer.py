...
class _ObserverBase(ObserverBase):
    ...
    Users should use `~nncs.quantization.observer.ObserverBase` as a base class
    ...
    def _calculate_qparams(
        ...
        include_zero = getattr(self, "include_zero", True)
        if include_zero:
            ...
            _min_val_neg = torch.min(min_val, torch.zeros_like(min_val))
            _max_val_pos = torch.max(max_val, torch.zeros_like(max_val))

            mask = (max_val_pos - min_val_neg) < 1e-5
            min_val_neg[mask] = _min_val_neg[mask]
            max_val_pos[mask] = _max_val_pos[mask]

        device = min_val_neg.device
        scale = torch.ones(min_val_neg.size(), dtype=torch.float32, device=device)
        zero_point = torch.zeros(min_val_neg.size(), dtype=torch.long, device=device)

        if self.qscheme == torch.per_tensor_symmetric:
            max_val_pos = torch.max(-min_val_neg, max_val_pos)
            if quant_min in [-128]:
                scale = max_val_pos.item() / (float(quant_max - quant_min - 1.0) / 2)
            else:
                scale = max_val_pos / (float(quant_max - quant_min) / 2)

            if type(scale) in [float]:
                scale = max(scale, self.eps.item())
            else:
                scale = torch.max(scale, self.eps)
        elif self.qscheme == torch.per_channel_symmetric:
            max_val_pos = torch.max(-min_val_neg, max_val_pos)
            if quant_min in [-128]:
                scale = max_val_pos / (float(quant_max - quant_min - 1.0) / 2)
            else:
                scale = max_val_pos / (float(quant_max - quant_min) / 2)
            scale = torch.max(scale, self.eps)
        ...
        if type(scale) in [float]:
            scale = torch.tensor([float(scale)], dtype=torch.float64, device=device)
        if type(scale) not in [float] and len(scale.shape) == 0:
...
class MovingAverageMinMaxObserver(MinMaxObserver):
    ...
    :class:`~nncs.quantization.observer.MinMaxObserver`.
...
class MovingAveragePercentileObserver(MinMaxObserver):
    def __init__(
        self,
        percentile=0.99,
        averaging_constant=0.01,
        ...
        self.averaging_constant = averaging_constant
        self.include_zero = include_zero
        super(MovingAveragePercentileObserver, self).__init__(
            dtype=dtype,
            qscheme=qscheme,
            reduce_range=reduce_range,
            quant_min=quant_min,
    ...
    def forward(self, x_orig):
        ...
        max_val = self.max_val
        nn = x_orig.numel()
        n_pick = max(int((1.0 - self.percentile) * nn), 1)

        if min_val == float("inf") and max_val == float("-inf"):
            # min_val, max_val = torch._aminmax(x)
            max_val, _ = torch.topk(x_flatten, n_pick, largest=True)
            max_val = max_val[-1]
            min_val, _ = torch.topk(x_flatten, n_pick, largest=False)
            min_val = min_val[-1]
        else:
            # min_val_cur, max_val_cur = torch._aminmax(x)
            max_val_cur, max_index = torch.topk(x_flatten, n_pick, largest=True)
            max_val_cur = max_val_cur[-1]
            min_val_cur, min_index = torch.topk(x_flatten, n_pick, largest=False)
            min_val_cur = min_val_cur[-1]
            min_val = min_val + self.averaging_constant * (min_val_cur - min_val)
            max_val = max_val + self.averaging_constant * (max_val_cur - max_val)
        self.min_val.resize_(min_val.shape)
        self.max_val.resize_(max_val.shape)
...
class PerChannelMinMaxObserver(_ObserverBase):
    ...
    :class:`~nncs.quantization.observer.MinMaxObserver`, with the difference
...
class MovingAveragePerChannelMinMaxObserver(PerChannelMinMaxObserver):
    ...
    The quantization parameters are computed the same way as in
    :class:`~nncs.quantization.observer.MovingAverageMinMaxObserver`, with the
...
class MovingAveragePerChannelPercentileObserver(PerChannelMinMaxObserver):
    def __init__(
        self,
        percentile=0.99,
        averaging_constant=0.01,
        ...
        super(MovingAveragePerChannelPercentileObserver, self).__init__(
            ch_axis=ch_axis,
            dtype=dtype,
            qscheme=qscheme,
            reduce_range=reduce_range,
    ...
    def forward(self, x_orig):
        ...
        y = x.permute(new_axis_list)
        y = torch.flatten(y, start_dim=1)
        n_pick = max(int((1.0 - self.percentile) * y.shape[1]), 1)

        if min_vals.numel() == 0 or max_vals.numel() == 0:
            # min_vals, max_vals = torch._aminmax(y, 1)
            max_vals, _ = torch.topk(y, n_pick, dim=1)
            min_vals, _ = torch.topk(y, n_pick, dim=1, largest=False)
            max_vals = max_vals[:, -1]
            min_vals = min_vals[:, -1]
        else:
            # min_vals_cur, max_vals_cur = torch._aminmax(y, 1)
            max_vals_cur, _ = torch.topk(y, n_pick, dim=1)
            min_vals_cur, _ = torch.topk(y, n_pick, dim=1, largest=False)
            max_vals_cur = max_vals_cur[:, -1]
            min_vals_cur = min_vals_cur[:, -1]
            min_vals = min_vals + self.averaging_constant * (min_vals_cur - min_vals)
            max_vals = max_vals + self.averaging_constant * (max_vals_cur - max_vals)
        self.min_vals.resize_(min_vals.shape)
        self.max_vals.resize_(max_vals.shape)
...
class HistogramObserver(_ObserverBase):
    ...
    3. Compute the scale and zero point the same way as in the
        :class:`~nncs.quantization.MinMaxObserver`
    """
...
def _is_observer_script_module(mod, obs_type_name):
    ...
    if isinstance(mod, torch.jit.RecursiveScriptModule):
        # qualified name looks like '__torch__.nncs.quantization.observer.___torch_mangle_2.MinMaxObserver'
...
def _is_activation_post_process(module):
    return (
        isinstance(module, nncs.quantization.ObserverBase)
        or isinstance(module, nncs.quantization.FakeQuantize)
        or _is_observer_script_module(module, "nncs.quantization.observer")
    )
...
def _is_per_channel_script_obs_instance(module):
    if isinstance(module, torch.jit.RecursiveScriptModule):
        return _is_observer_script_module(
            module, "nncs.quantization.observer.PerChannelMinMaxObserver"
        ) or _is_observer_script_module(
            module, "nncs.quantization.observer.MovingAveragePerChannelMinMaxObserver"
        )
    return False
...
def load_observer_state_dict(mod, obs_dict):
    ...
    using nncs.quantization.get_observer_state_dict
    """
    missing_keys: List[str] = []
    unexpected_keys: List[str] = []
    for name, module in mod.named_modules():
        ...
        if _is_activation_post_process(module):
            if _is_per_channel_script_obs_instance(module):
                # For per-channel observers we need to call
                # a custom load_from_state_dict to resize the tensor.
                # However this is not called when the module
                # is scripted and we end up calling the default one in module.py
