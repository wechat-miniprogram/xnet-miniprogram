...
def _calculate_scale_grad(dY, X, X_fq, X_q, scale, zero_point, q_min, q_max, device):
    ...
    indicate_middle_scale = (
        torch.ones(indicate_small_scale.shape, device=device)
    ...
    dScale = (
        indicate_small_scale * dScale_small
...
def _calculate_zero_point_grad(
    dY, X, X_fq, X_q, scale, zero_point, q_min, q_max, device
):
    ...
    indicate_unsaturate_zp = (
        torch.ones(indicate_saturate_zp.shape, device=device) - indicate_saturate_zp
    )
    ...
    dZeroPoint = (
        indicate_saturate_zp * dZeroPoint_saturate
...
class _LearnableFakeQuantizePerTensorOp(torch.autograd.Function):
    ...
    def forward(ctx, X, scale, zero_point, q_min, q_max, grad_factor):
        ...
        X_fq = torch.fake_quantize_per_tensor_affine(X, scale_val, zp_val, q_min, q_max)
...
class _LearnableFakeQuantizePerChannelOp(torch.autograd.Function):
    ...
    def backward(ctx, dY):
        ...
        dScale = _calculate_scale_grad(
            dY, X, X_fq, X_q, scale_vec, zp_vec, q_min, q_max, device
        ).sum(axis_for_reduction)
...
class _LearnableFakeQuantize(nn.Module):
    ...
    * attr: `learning_enabled` defines the flag for enabling backpropagation for scale and zero point.
    ...
    def forward(self, X):
        ...
        if self.fake_quant_enabled[0] == 1:
            if self.learning_enabled[0] == 1:
                ...
                # zero_point = torch.clamp(self.zero_point, self.quant_min, self.quant_max)
                scale = torch.where(
                    self.scale < 0,
                    torch.min(self.scale, -self.eps),
                    torch.max(self.scale, self.eps),
                )
                if self.qscheme in (
                    ...
                    zero_point_round = RoundWithGradient.apply(self.zero_point)
                    zero_point = torch.clamp(
                        zero_point_round, self.quant_min, self.quant_max
                    )
                    ...
                    X = torch._fake_quantize_learnable_per_tensor_affine(
                        X, scale, self.zero_point, self.quant_min, self.quant_max
                    )
            else:
                if (
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        ...
        super(_LearnableFakeQuantize, self)._save_to_state_dict(
        ...
        for name in local_state:
            ...
            if key in state_dict:
                ...
                if name == "scale":
                    ...
                    self.zero_point.data.copy_(val)
...
def disable_param_learning(mod):
    if isinstance(mod, _LearnableFakeQuantize):
...
def enable_fake_quant(mod):
    if isinstance(mod, _LearnableFakeQuantize):
        mod.toggle_fake_quant(True)


def disable_fake_quant(mod):
    if isinstance(mod, _LearnableFakeQuantize):
        mod.toggle_fake_quant(False)


def enable_observer(mod):
    if isinstance(mod, _LearnableFakeQuantize):
        mod.toggle_observer_update(True)


def disable_observer(mod):
    if isinstance(mod, _LearnableFakeQuantize):
        mod.toggle_observer_update(False)
