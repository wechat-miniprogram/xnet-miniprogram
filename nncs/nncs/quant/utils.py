...
def get_swapped_custom_module_class(
    ...
    assert type(custom_module) in class_mapping, (
        "did not found corresponding observed "
...
def get_quant_type(qconfig):
    ...
    if weight.dtype in static_dtypes:
        if activation.dtype in static_dtypes:
            ...
            hasattr(activation, "compute_dtype")
            and activation.compute_dtype in static_dtypes
        ):
    ...
    raise Exception(
        "Unrecognized dtype combination in get_quant_type: activation({}),"
